...
def sinkhorn_on_log_domain_GPU_memory_optimize(a, b, M, reg, epsilon0 = 1e6, numItermax=10,stopThr=1e-9):
    ...
    def get_K(alpha, beta,reg):
        ...
        M_ /= -reg
        M_ = M_.exp()
        res = M_.sum(1,keepdim = True)
        M_ = torch.detach(M_)
        M_ = M_.cpu()
        del M_      
        res = torch.detach(res)
        return res
    ...
    while loop:
        ...
        for i in range(numIterinner[ind]):
            ...
            min_f,_ = torch.min(M_, dim = 1,  keepdim = True)
            ...
            M2 = torch.exp(M_)
            M2[subset] = 0
            temp = M2.sum(1,keepdim = True)

            M2 = M2.cpu()
            del M2
            M_ = M_.cpu()
            del M_
            temp_ = torch.clone(temp)
            temp_ = torch.detach(temp_)
            temp = temp.cpu()
            del temp
            ...
            f = regk * torch.log(a) -\
            regk * torch.log(temp_) + min_f

            
            M_ = torch.clone(M)  
            M_ -=  f

            min_g,_ = torch.min(M_, dim = 0, keepdim = True)
            M_ -= min_g
            M_ /= -regk 
            M2 = torch.exp(M_)
            M2[subset] = 0
            temp = M2.sum(0,keepdim = True)
            ...
            M_ = M_.cpu()
            if cptt == 1:
                subset = (M_ < torch.log(torch.tensor([1e-10])))
            del M_
            temp_ = torch.clone(temp)
            temp_ = torch.detach(temp_)
            temp = temp.cpu()
...
def sinkhorn_on_log_domain_GPU_keops_high_dimension(d, mu, nv, x,y,  reg, epsilon0 = 1e6, numItermax=10,stopThr=1e-9,debug=0,ranges = None,
                                    warmf=None,warmg=None):
    ...
    log_likelihood = generic_logsumexp(
       '(-eps * (SqNorm2(x-y)-g))  ', # Formula
        ...
        'x = Vi('+str(d)+')',              # 1st input: dim-1 vector per line
